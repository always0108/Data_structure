...
int  find_rule(char ch,char s[256]);//查找规则是否已存在
int  find_endlanguage(char ch,char s[256]);//查找最终语言有无解释
...
//查找规则是否已存在
int  find_rule(char ch,char s[256]){
    FILE *fp;
    fp = fopen("Rule","r");
    if(!fp){
...
//查找最终语言有无解释
int  find_endlanguage(char ch,char s[256]){
    FILE *fp;
...
void find_end_rule(char s1[256],char s2[256]){
    int i,j=0;
    char s3[256];
